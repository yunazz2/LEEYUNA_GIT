...
<%
    // JSP implementation ResetPassword - Purpose is to allow a personalized Reset Password form per organization
    Logger _logger = LogUtilities.getInstance().getApplicationLogger(BaseService.class);
    Logger _securityLogger = LogUtilities.getInstance().getApplicationLogger(BaseService.class);

    _logger.debug("Executing ResetPassword.jsp");
    ...
    String userName = StringUtilities.decodeString(request.getParameter(RESTAPIConstants.THINGWORX_FORM_USERNAME));
    String token = request.getParameter(RESTAPIConstants.SUMMARY_CHARACTERISTIC);
    ...
    try {
        ...
        if(StringUtilities.isNullOrEmpty(userName) || StringUtilities.isNullOrEmpty(token)) {
            throw new InvalidRequestException("Not Authorized to Reset the Password",RESTAPIConstants.StatusCode.STATUS_INTERNAL_ERROR);
        }
        ...
        if(_securityLogger.isWarnEnabled()) {
	        _securityLogger.warn("[{}] of: [{}] is on the reset password page", ESAPIUtilities.cleanLoggedString(userName) , ESAPIUtilities.cleanLoggedString(orgName));
        }
        ...
        try {
            ...
            response.sendError(ire.getStatusCode().httpCode(), RESTAPIConstants.StatusCode.STATUS_BAD_REQUEST.getStatusMessage());
        } catch(Exception ioe) {
...
<html>
<head>
    <title data-i18n="tw.login.labels.title-reset-password"></title>
...
<%  }%>
        ...
        <div class="org-info">
            <p class="orgDescription" data-i18n="tw.login.labels.enter-new-password"></p>
        </div>
        <div class="reset-form">

            <%--changed to absolute path to fix customer issue--%>
            <form method="POST" action="/<%=RESTAPIConstants.WEBAPP_NAME%>/<%=RESTAPIConstants.ACTION_CHANGE_PASSWORD%>">
                <input class="text-field" type="PASSWORD" name="<%=RESTAPIConstants.THINGWORX_FORM_NEW_PASSWORD%>" value="" data-i18n="[title]tw.login.labels.password-title;[placeholder]tw.login.labels.password">
                <input class="text-field" type="PASSWORD" name="<%=RESTAPIConstants.THINGWORX_FORM_NEW_PASSWORD_CONFIRM%>" value="" data-i18n="[title]tw.login.labels.password-title;[placeholder]tw.login.labels.confirmation">
                <input class="reset-buttons" type="submit" name="Login" data-i18n="[value]tw.login.buttons.submit">
                <%@include file="ErrorMessage.jsp" %>
                <%@include file="FormCSRFCookie.jsp" %>
                <input type="hidden" name="<%=ESAPIUtilities.getEncoder().encodeForHTMLAttribute(RESTAPIConstants.THINGWORX_FORM_USERNAME)%>" value="<%=ESAPIUtilities.getEncoder().encodeForHTMLAttribute(userName)%>">
                <input type="hidden" name="<%=ESAPIUtilities.getEncoder().encodeForHTMLAttribute(RESTAPIConstants.SUMMARY_CHARACTERISTIC)%>" value="<%=ESAPIUtilities.getEncoder().encodeForHTMLAttribute(token)%>">
